...
# Training Loop
num_epochs = 100
masking_rates = [0.50, 0.75, 0.25]  # Different masking rates to be used
for epoch in range(num_epochs):
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch + 1}/{num_epochs}', unit='batch') as pbar:
        for batch_idx, images in enumerate(train_loader):
            ...
            outputs, mask = model(images, mask_ratio=mask_rate)
            
            # Compare the predicted masked patches with the original patch embeddings
            original_patches = model.patch_embed(images)
            
            print("outputs", outputs.shape)
            print("original_patches", original_patches.shape)
            loss = criterion(outputs, original_patches)
