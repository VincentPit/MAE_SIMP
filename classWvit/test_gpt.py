...
def load_checkpoint(
    ...
    checkpoint_name="model_epoch_7_mask_75.pth",
):
...
def save_comparison(images, predictions, image_paths, save_dir="results"):
    ...
    for img, pred, img_path in zip(images, predictions, image_paths):
        ...
        for i, color in enumerate(["R", "G", "B"]):
            print(
                f"{color}-channel of Ground Truth: min={img[:, :, i].min()}, max={img[:, :, i].max()}"
            )
            print(
                f"{color}-channel of Prediction: min={pred[:, :, i].min()}, max={pred[:, :, i].max()}"
            )
